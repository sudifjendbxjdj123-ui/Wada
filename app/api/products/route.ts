...
const EXCLUDE_ACCENT = /\b(parapluie|umbrella|serviette|towel|gant(s)?\s+(de\s+ski|de\s+pluie)|cagoule|tour\s+de\s+cou|masque|porte[-\s]?cl[ée]|trousse|étui|protection|housse|sac\s+(à\s+linge|de\s+rangement)|jet|bouteille|gourde|stylo|carnet|bouton\s+de\s+manchette|cufflink|manchette|noeud\s+papillon|bow[\s-]?tie|pochette\s+de\s+costume|pocket\s+square|bretelles|braces?|suspenders|cravate|necktie|herringbone\s+tie|soie\s+cravate|cadre|décoration|décor|home\s+decor|art|tableau|affiche|poster|mur|wall|plante|plant|vase|pot|coussin|cushion|lampe|light|miroir|mirror|horloge|clock)/i;
